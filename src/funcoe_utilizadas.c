...
// Inicializa o jogo, setando o tamanho da tela e FPS, e retornando o ponteiro duplo
...
void atirar_boss(Personagens *boss, Image sprite, float dir_x, float dir_y)
{
    ...
    // define tamanho da hitbox da bala
...
void atirar(Personagens *player, Image sprite)
{
    ...
    if (mag < 0.0001f)
    {
        dx = 1.0f; dy = 0.0f; mag = 1.0f; // evita divisão por zero
    }
